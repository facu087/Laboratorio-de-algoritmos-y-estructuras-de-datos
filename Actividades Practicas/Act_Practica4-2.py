...
suma_pares = sum(numeros for numeros in lista_numeros if numeros %2 ==0)
suma_impares = sum(numeros for numeros in lista_numeros if numeros %2 ==1)


print(f"La suma de los numeros pares es de {suma_pares}")
print(f"La suma de los numeros impares es de {suma_impares}")
...
suma_cuadrados=sum(numero**2 for numero in mi_lista)
...
for indice, elemento in enumerate (palabra):
    lista_indices=[indice, elemento]
    print(lista_indices)
...
for indice, nombre in enumerate(lista_nombres):
    if nombre.startswith("M"):
        print(f'El nombre "{nombre}" comienza con "M" y está en el índice {indice}')
